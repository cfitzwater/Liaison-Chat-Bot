...
admin_gui = Admin(app, name='Liaison DB Admin')
...
def add_form():
    if 'user_id' not in session: return redirect(url_for('login'))
    return render_template('add_item.html', full_name=session.get('first_name'), email=session.get('user_email'))

@app.route('/save_item', methods=['POST'])
def save_item():
    notes = request.form.get('notes')
    entry_id = str(uuid.uuid4())
    new_entry = LibraryEntry(id=entry_id, notes=notes)
